...
public class RspGroupUserSettingListProcessor extends AbstractProcessor {
    ...
    public void onMessage(TMMessage message) throws Throwable {
        final Messages.GroupUserSettingRsp rspList = Messages.GroupUserSettingRsp.parseFrom(message.getBody());

        if (logger.isInfoEnabled()) {
            logger.info(LogFormat.format(LogFormat.LogModule.SERVICE, LogFormat.Operation.PROCESS, "{}"), rspList);
        }

        for (Messages.UpdateGroupUserSettingRsp rsp : rspList.getUpdateGroupUserSettingRspList()) {
